 ...
 * Description: A simple plugin to save contact form submissions in the database, designed for the Elementor Form Module. Add "no-record" in the name of form if not need to store submissions.
		...
		if(!empty( $_POST['sb_elem_cfd_export'] )) {
			if(wp_verify_nonce( $_POST['sb_elem_cfd_export'], 'sb_elem_cfd_export' )) {
				...
				if(isset( $_REQUEST['form_id'] )) {
					$form_id_fixed = sanitize_text_field($_REQUEST['form_id']);
					if($rows = sb_elem_cfd_get_export_rows_by_form_id( $form_id_fixed )) {

						header( 'Content-Type: application/csv' );
						header( 'Content-Disposition: attachment; filename=' . sanitize_title( $form_id_fixed ) . '.csv' );
						header( 'Pragma: no-cache' );
						echo implode( "\n", $rows );
						die;
					}
...
function sb_elem_cfd_new_record($record, $form_class) {
	...
	if($fields = $record->get_formatted_data()) {
		...
		if (str_contains($record->get_form_settings( 'form_name' ), 'no-record')) { 
    return;
}
